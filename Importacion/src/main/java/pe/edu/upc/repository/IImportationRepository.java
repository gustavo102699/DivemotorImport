...
public interface IImportationRepository extends JpaRepository<Importation, Long>{
	...
	@Query("select i from Importation i join fetch i.import_details id join fetch id.product where i.id=?1")
	Optional<Importation> fetchByImportIdWhithImportDetailsWithProduct(Long id);
	
	@Query("select i from Importation i join fetch i.legal_files lf join fetch lf.template where i.id=?1")
